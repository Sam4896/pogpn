...
TRANSFORM_WARNING = (
    "Using an {ttype} transform with `BoTorchVariationalGP`. If this "
...
class ApproximateGPyTorchModel(GPyTorchModel):
    r"""Botorch wrapper class for various (variational) approximate GP models in
    GPyTorch.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        model: Instance of gpytorch.approximate GP models. If omitted,
            constructs a `_BoTorchVariationalGP`.
        ...
        args: Optional positional arguments passed to the
            `_BoTorchVariationalGP` constructor if no model is provided.
        ...
        self.model = (
            _BoTorchVariationalGP(num_outputs=num_outputs, *args, **kwargs)
            if model is None
            else model
        )
    ...
    def eval(self) -> Self:
        r"""Put the model in `eval` mode."""
        return Module.eval(self)  # type: ignore
    ...
    def train(self, mode: bool = True) -> Self:
        ...
        """
        return Module.train(self, mode=mode)  # type: ignore
...
class _BoTorchVariationalGP(ApproximateGP):
    ...
    def __init__(
        self,
        train_X: Tensor,  # noqa: N803
        train_Y: Tensor | None = None,  # noqa: N803
        num_outputs: int = 1,
        ...
        if covar_module is None:
            covar_module = get_covar_module_with_dim_scaled_prior(
                ...
                batch_shape=self._aug_batch_shape,  # type: ignore
        ...
        if num_outputs > 1:
            variational_strategy_instance = LMCVariationalStrategy(
                base_variational_strategy=variational_strategy_instance,
                num_tasks=num_outputs,
                num_latents=num_outputs,
            )
        super().__init__(variational_strategy=variational_strategy_instance)

        self.mean_module = (
            ConstantMean(batch_shape=self._aug_batch_shape).to(train_X)  # type: ignore
    ...
    def forward(self, X) -> MultivariateNormal:
        ...
        latent_dist = MultivariateNormal(mean_x, covar_x)  # type: ignore
        return latent_dist


class BoTorchVariationalGP(ApproximateGPyTorchModel):
    ...
    def __init__(
        ...
        train_X: Training inputs (due to the ability of the SVGP to sub-sample
            ...
            num_classes: Number of classes for the categorical outputs.
        ...
        if train_Y is not None:
            ...
            self._validate_tensor_args(X=transformed_X, Y=train_Y)  # type: ignore
            validate_input_scaling(
                train_X=transformed_X,
                train_Y=train_Y,  # type: ignore
                check_nans_only=covar_module is not None,
            )
            if train_Y.shape[-1] != num_outputs:  # type: ignore
                num_outputs = train_Y.shape[-1]  # type: ignore
            ...
            if num_outputs == 1:
                ...
                if num_classes is not None:
                    likelihood = SoftmaxLikelihood(
                        num_features=num_outputs, num_classes=num_classes
                    )
        ...
        self.model.train_inputs = [transformed_X]  # type: ignore
        if train_Y is not None:
